create or replace PACKAGE BODY xxhtz_visioninv_extract AS
  ...
  * 1.5     17-Jan-2025  Subhajit Das  INC002269052
  *************************************************************************/
    ...
    PROCEDURE main (
       -- p_oic_instance_id IN NUMBER, --Commented as part of V 1.4
        ...
        ORDER BY
            ...
            line_number;
-- added in v1.5
        CURSOR rec_ext_cur IS
                SELECT invoice_id, area_nummber, po_number, error_desc
                FROM xxhtz_ap_vision_extract_tbl
                WHERE last_update_date = SYSDATE
                  AND error_flag <> 'Y'
                  AND region = p_bu_name;
        
        TYPE rec_ext_cur_type IS TABLE OF rec_ext_cur%ROWTYPE;
        rec_ext_cur_tab rec_ext_cur_type;
-- end v1.5
                           ...
                           -- added by joy das on 4th sept .
                               ...
                               --  || v_vendor_name1
		                         ...
		                         */
                                    ...
                                    /*IF rec_cur_hdr.sent_status = 'Sent' THEN  -- commented in v1.5
                                        v_inv_status_code := 'P';
                                    END IF;
...
------
                              ...
                              -- End modification for IC R12
                                -- v_new_status := 'Sent';
                                    END IF;*/ -- v1.5
                            --Added in v1.5
                                        v_inv_status_code := CASE
                                            WHEN rec_cur_hdr.sent_status = 'Sent' THEN 'P'
                                            WHEN rec_cur_hdr.sent_status IS NULL THEN 'A'
                                            WHEN rec_cur_hdr.sent_status = 'Error' AND rec_cur_hdr.payment = 'Y' THEN 'A'
                                            WHEN rec_cur_hdr.sent_status = 'Error' AND rec_cur_hdr.payment = 'N' THEN 'A'
                                            ELSE v_inv_status_code
                                        END;
                                
                                        -- Update xxhtz_ap_vision_extract_tbl based on conditions
                                        IF (rec_cur_hdr.sent_status IS NULL OR rec_cur_hdr.sent_status = 'N')
                                           AND (rec_cur_hdr.payment = 'Y' OR rec_cur_hdr.payment = 'P') THEN
                                            UPDATE xxhtz_ap_vision_extract_tbl
                                            SET sent_status = 'Paid',
                                                payment_date = rec_cur_hdr.check_date,
                                                payment_flag = rec_cur_hdr.payment,
                                                last_update_date = SYSDATE
                                            WHERE invoice_id = rec_cur_hdr.invoice_id;
                                        ELSIF (rec_cur_hdr.sent_status IS NULL AND rec_cur_hdr.payment = 'N') 
                                               OR rec_cur_hdr.sent_status = 'Error' THEN
                                            UPDATE xxhtz_ap_vision_extract_tbl
                                            SET sent_status = 'Sent',
                                                last_update_date = SYSDATE
                                            WHERE invoice_id = rec_cur_hdr.invoice_id;
                                        ELSIF rec_cur_hdr.sent_status = 'Sent' AND rec_cur_hdr.payment = 'Y' THEN
                                            UPDATE xxhtz_ap_vision_extract_tbl
                                            SET sent_status = 'Paid',
...
-- Added in v1.5            
            BEGIN
                ...
                SELECT 
                    MAX(CASE WHEN line_type = 'ITEM' AND PO_SEGMENT1 IS NOT NULL THEN po_segment1 END) AS po_segment1,
                    MAX(CASE WHEN lookup_code IS NOT NULL THEN lookup_code END) AS lookup_code
                INTO 
                    v_po_number, 
                    v_po_area_num
                FROM 
...
-- Added in v1.5
    BEGIN
        OPEN rec_ext_cur;
        FETCH rec_ext_cur BULK COLLECT INTO rec_ext_cur_tab;
        CLOSE rec_ext_cur;
        
         FOR i IN rec_ext_cur_tab.FIRST .. rec_ext_cur_tab.LAST LOOP
            SELECT COUNT(*)
            INTO v_line_count
            FROM xxhtz_invoice_lines_stg_tbl
            WHERE invoice_id = rec_ext_cur_tab(i).invoice_id
            ...
            IF v_line_count = 0 THEN
                ...
                WHERE invoice_id = rec_ext_cur_tab(i).invoice_id;
            ELSE
                IF rec_ext_cur_tab(i).area_nummber IS NULL OR rec_ext_cur_tab(i).area_nummber = '00000' THEN
                    UPDATE xxhtz_ap_vision_extract_tbl
                    SET sent_status = NULL,
                        error_desc = 'Area Number Missing'
                    WHERE invoice_id = rec_ext_cur_tab(i).invoice_id;
                ...
                IF rec_ext_cur_tab(i).po_number IS NULL OR rec_ext_cur_tab(i).po_number LIKE '0000%' THEN
                    UPDATE xxhtz_ap_vision_extract_tbl
                    SET sent_status = NULL,
                        error_desc = rec_ext_cur_tab(i).error_desc || ' PO Number Missing'
                    WHERE invoice_id = rec_ext_cur_tab(i).invoice_id;
                END IF;
            END IF;
        END LOOP;
